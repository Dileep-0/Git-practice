...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(1, len(df)):
        ...
        high_ce, close_ce, low_ce, open_ce = current_row['high'], current_row['close'], current_row['low'], current_row['open']
        high_pe, close_pe, low_pe, open_pe = current_row['high.1'], current_row['close.1'], current_row['low.1'], current_row['open.1']

        body_ce = abs(open_ce - close_ce)
        body_pe = abs(open_pe - close_pe)
        wik_ce = abs(open_ce - low_ce)
        wik_pe = abs(open_pe - low_pe)

        min_wik_ce = (wik_ce/body_ce)*100 < 10
        hammer_ce = (close_ce - low_ce) > 3*body_ce

        min_wik_pe = (wik_pe/body_pe)*100 < 10
        hammer_pe = (close_pe - low_pe) > 3*body_pe
        ...
        curr_ch_buy = change_in_buy
        curr_ch_sell = change_in_sell
        ...
        '''#same flow
        for_buy  = abs(buy_ratio) > abs(sell_ratio) and abs(ch_b_rtio) > abs(ch_s_rtio) and (diff_of_hor < 0.3 or diff_of_hor > 1) and (diff_of_ver > 1 or diff_of_ver < 0.3)
        for_sell = abs(sell_ratio) > abs(buy_ratio) and abs(ch_s_rtio) > abs(ch_b_rtio) and (diff_of_hor > 1 or diff_of_hor < 0.3) and (diff_of_ver > 1 or diff_of_ver < 0.3)
        #opposite flow
        buy_2 = (abs(buy_ratio) < abs(sell_ratio) and abs(ch_b_rtio) < abs(ch_s_rtio)) and (diff_of_hor < 0.3 or diff_of_hor > 1) and (diff_of_ver > 1 or diff_of_ver < 0.3)
        sell_2 = (abs(sell_ratio) < abs(buy_ratio) and abs(ch_s_rtio) < abs(ch_b_rtio)) and (diff_of_hor > 1 or diff_of_hor < 0.3) and (diff_of_ver > 1 or diff_of_ver < 0.3)'''

        
        #same flow
        for_buy  = abs(buy_ratio) > abs(sell_ratio) and abs(ch_b_rtio) > abs(ch_s_rtio) and ((abs(buy_ratio) > 1.48 and abs(sell_ratio) < 0.4) or diff_of_hor < 0.3) and ((abs(ch_b_rtio) > 1.48 and abs(ch_s_rtio) < 0.4) or diff_of_ver < 0.3)
        for_sell = abs(sell_ratio) > abs(buy_ratio) and abs(ch_s_rtio) > abs(ch_b_rtio) and ((abs(sell_ratio) > 1.48 and abs(buy_ratio) < 0.4) or diff_of_hor < 0.3) and ((abs(ch_s_rtio) > 1.48 and abs(ch_b_rtio) < 0.4) or diff_of_ver < 0.3)
        #opposite flow
        buy_2 = (abs(buy_ratio) < abs(sell_ratio) and abs(ch_b_rtio) < abs(ch_s_rtio)) and abs(buy_ratio) > 0.3 and abs(ch_b_rtio) > 0.3 and (diff_of_hor >= 1 or diff_of_ver >= 1)
        sell_2 = (abs(sell_ratio) < abs(buy_ratio) and abs(ch_s_rtio) < abs(ch_b_rtio)) and abs(sell_ratio) > 0.3 and abs(ch_s_rtio) > 0.3 and (diff_of_hor >= 1 or diff_of_ver >= 1)
        ...
        contract_signal_power_b = (contract_signal == 1 and signal_power > 1) or signal_power > 1.2
        contract_signal_power_s = (contract_signal == -1 and signal_power > 1) or signal_power > 1.2
        ...
        pattern = ''
        if for_buy or for_sell:
            pattern = 'same'
        elif buy_2 or sell_2:
            pattern = 'Opp'
        elif buy_3 or sell_3:
            pattern = 'Alt'
        
        
        if (ltp_check or ltp_check_2) and count_Y > 3 and (for_buy or buy_2 or buy_3 ) and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            ...
            buy_flag = True
            print(f"{current_row['last_traded_time']} - buy {pattern}")
            #print(ltp_price_ce/ltp_price_pe)

        elif (ltp_check or ltp_check_2) and count_Y == 3 and count_N >= 1 and (for_buy or buy_2 or buy_3 )  and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            and  ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
            (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
            ...
            if buy_flow_res > 3:
                buy_flag = True
                print(f"{current_row['last_traded_time']} - buy {pattern}")
                print(change_in_buy, p_ch_buy)
                print(change_in_sell, p_ch_sell)
                #print(ltp_price_ce/ltp_price_pe)
        ...
        if not buy_flag:
            if (ltp_check or ltp_check_2) and count_YES > 3  and (for_sell or sell_2 or sell_3) and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
                print(f"{current_row['last_traded_time']} - sell {pattern}")
                #print(ltp_price_ce/ltp_price_pe)

            elif (ltp_check or ltp_check_2) and count_YES == 3 and count_N >= 1  and (for_sell or sell_2 or sell_3) and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
        ...
        p_ch_buy, p_ch_sell = curr_ch_buy, curr_ch_sell
