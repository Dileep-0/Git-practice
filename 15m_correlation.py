...
current_time = df.index.min().floor('5min')
max_time = df.index.max()

while current_time <= max_time:
    # Define the rolling window: last 15 minutes from the current time
    start_time = max(df.index.min(), current_time - pd.Timedelta(minutes=10))
    end_time = min(df.index.max(), current_time + pd.Timedelta(minutes=4))
    

    # Select data in the rolling window
    rolling_group = df.loc[start_time:end_time]

    filtered_group = rolling_group[(rolling_group[['ltp', 'tot_buy_qty', 'tot_sell_qty']] != 0).all(axis=1)]

    if not filtered_group.empty:
    # Initialize a dictionary to store the correlation values for this timestamp
        correlation_values = {'last_traded_time': current_time.strftime('%d-%m-%Y %H:%M')}
        ...
        correlation_values['n_ltp_diff'] = rolling_group.iloc[-1]['ltp'] - rolling_group.iloc[0]['ltp']
        correlation_values['n_buy_diff'] = rolling_group.iloc[-1]['tot_buy_qty'] - rolling_group.iloc[0]['tot_buy_qty']
        correlation_values['n_sell_diff'] = rolling_group.iloc[-1]['tot_sell_qty'] - rolling_group.iloc[0]['tot_sell_qty']
        correlation_values['ce_ltp_diff'] = rolling_group.iloc[-1]['ltp.1'] - rolling_group.iloc[0]['ltp.1']
        correlation_values['ce_buy_diff'] = rolling_group.iloc[-1]['tot_buy_qty.1'] - rolling_group.iloc[0]['tot_buy_qty.1']
        correlation_values['ce_sell_diff'] = rolling_group.iloc[-1]['tot_sell_qty.1'] - rolling_group.iloc[0]['tot_sell_qty.1']
        correlation_values['pe_ltp_diff'] = rolling_group.iloc[-1]['ltp.2'] - rolling_group.iloc[0]['ltp.2']
        correlation_values['pe_buy_diff'] = rolling_group.iloc[-1]['tot_buy_qty.2'] - rolling_group.iloc[0]['tot_buy_qty.2']
        correlation_values['pe_sell_diff'] = rolling_group.iloc[-1]['tot_sell_qty.2'] - rolling_group.iloc[0]['tot_sell_qty.2']
