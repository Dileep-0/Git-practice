...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(len(df)-1):
        ...
        if i == 0:
            ...
            ce_high, ce_low = current_row['high.1'], current_row['low.1']
            pe_open, pe_close = current_row['open_3'], current_row['close_3']
            pe_high, pe_low = current_row['high.2'], current_row['low.2']
            n_high, n_low = current_row['high'], current_row['low']
            open_vix = current_row['open']
            close_vix = current_row['close']
            high_vix = current_row['high.3']
        ...
        nif_body = n_open - n_close
        vix_body = open_vix - close_vix
        length = n_high - n_low
        buy_atp_eff = n_low - 50
        sell_atp_eff = n_high + 50
        body_per =  abs(length)/2 < abs(nif_body) 
        ltp_check_2 = abs(length) > 14 and body_per and abs(ltp_price) > 12

        candle_patterns = candle_pattern(n_open, n_high, n_low, n_close)
        roc_vp1n = (0.01*abs(nif_body))/abs(vix_body)
        roc_wrt_prev_roc = abs(nif_body)/prev_roc_vp1n/100
        
        roc = -1 if abs(vix_body) < roc_wrt_prev_roc else 1
        opp_can = nif_body * prev_nif_body < 0
        #print("roc", roc_vp1n, roc_wrt_prev_roc, roc)
        ...
        if i > 0:
            ...
            ce_upper = (abs(ce_high - ce_close) / abs(ce_close - ce_open) * 100) < 30
            pe_upper = (abs(pe_high - pe_close) / abs(pe_close - pe_open) * 100) < 30
        ...
        if i < 8:
            ...
            buy_prev_5_low = buy_low - buy_candle_len*1 < min(buy_prev_low[:i+1])
            sell_prev_5_low = sell_low - sell_candle_len*1 < min(sell_prev_low[:i+1])

        else:
            previous_5_b_candles = vertical_buys[i-4:i+1]
            previous_5_s_candles = vertical_sells[i-4:i+1]
            buy_prev_5_low = buy_low - buy_candle_len*1 < min(buy_prev_low[i-8:i])
            sell_prev_5_low = sell_low - sell_candle_len*1 < min(sell_prev_low[i-8:i])
        ...
        if i>0:
            ...
            len_prev = len(str(abs(int(prev_buy_sum))))
            len_curr = len(str(abs(int(buy_sum))))
            len_prev2 = len(str(abs(int(prev_sell_sum))))
            len_curr2 = len(str(abs(int(sell_sum))))
            
            buy_3_check = ((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) and \
                (len_prev >= 5 and len_curr >= 5 and len_prev2 >= 5 and len_curr2 >= 5 and (abs(len_prev - len_curr) == 2 or abs(len_prev2 - len_curr2) == 2))
            
            sell_3_check = ((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) and \
                (len_prev >= 5 and len_curr >= 5 and len_prev2 >= 5 and len_curr2 >= 5 and (abs(len_prev - len_curr) == 2 or abs(len_prev2 - len_curr2) == 2))
        

        if  ltp_check and count_Y > 3  and (buy_vix or buyer_ratio) and (contract_sentiment == 1 or buy_2_check or buy_3_check) \
            and ce_upper and buy_prev_5_low and avg_trade_buy and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
        ...
        elif ltp_check and count_Y == 3 and count_N >= 1 and (buy_vix or buyer_ratio) and (contract_sentiment == 1 or buy_2_check or buy_3_check) \
            and ce_upper and buy_prev_5_low and avg_trade_buy and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
            (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
        ...
        if not buy_flag:
            if ltp_check and count_YES > 3 and (sel_vix or seller_ratio) and (contract_sentiment == -1 or sell_2_check or sell_3_check) \
                and pe_upper and sell_prev_5_low and avg_trade_sell and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
            ...
            elif ltp_check and count_YES == 3 and count_N >= 1  and (sel_vix or seller_ratio) and (contract_sentiment == -1 or sell_2_check or sell_3_check)\
                and pe_upper and sell_prev_5_low and avg_trade_sell and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
        ...
        prev_roc_vp1n = roc_vp1n
        prev_nif_body = nif_body
