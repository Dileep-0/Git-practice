...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(len(df)-1):
        ...
        if i > 0:
            ...
            avg_trade_buy = (n_atp < (nifty_close + 20) and buy_atp_eff < n_atp ) or ((n_high + 75) < n_atp)
            avg_trade_sell = (n_atp > (nifty_close - 20) and sell_atp_eff > n_atp) or ((n_low - 75) > n_atp)
        ...
        if ltp_check and count_Y > 3 and  avg_trade_buy\
        ...
        elif ltp_check and count_Y == 3 and count_N >= 1 and  avg_trade_buy\
            and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
            (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
        ...
        if not buy_flag:
            if ltp_check and count_YES > 3 and  avg_trade_sell\
                and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
                print(f"{current_row['last_traded_time']} - sell")

            elif ltp_check and count_YES == 3 and count_N >= 1 and  avg_trade_sell\
                and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
