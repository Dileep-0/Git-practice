...
grouped = {}
pattern = r"(\d{2}-\d{2}-\d{4})_NIFTY.*?(FUT|CE|PE)"
...
for datekey, files in sorted_grouped.items():
    ...
    common_times = set(agg1['last_traded_time']) & set(agg2['last_traded_time']) & set(agg3['last_traded_time'])
...
# Merge on time
    final_df = agg1.merge(agg2, on='last_traded_time').merge(agg3, on='last_traded_time')
    formatted_date = pd.to_datetime(datekey, dayfirst=True).strftime('%Y-%m-%d')
    #final_df.insert(1, 'date', formatted_date)

    signals = []
    for i, row in final_df.iterrows():
        y_count = sum(1 for v in row.values if v == 'Y')
        yes_count = sum(1 for v in row.values if v == 'YES')
        n_count = sum(1 for v in row.values if v == 'N')
    ...
    combined_df.to_csv(os.path.join(folder_path, "detailed_aggregated_loss_3x.csv"), index=False)
