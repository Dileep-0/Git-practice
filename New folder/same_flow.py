...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(1, len(df)):
        ...
        if (ltp_check or ltp_check_2) and count_Y > 3 and (for_buy) and contract_signal_power_b and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
        ...
        elif (ltp_check or ltp_check_2) and count_Y == 3 and count_N >= 1 and (for_buy) and contract_signal_power_b and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
            (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
        ...
        if not buy_flag:
            if (ltp_check or ltp_check_2) and count_YES > 3  and (for_sell) and contract_signal_power_s and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
                print(f"{current_row['last_traded_time']} - sell {pattern}")
                #print(ltp_price_ce/ltp_price_pe)

            elif (ltp_check or ltp_check_2) and count_YES == 3 and count_N >= 1  and (for_sell) and contract_signal_power_s and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
