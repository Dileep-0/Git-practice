...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(1, len(df)):
        ...
        if ltp_ce_check:
            ...
            #print(ltp_diff_ce)
            ltp_ce_check = 0.25 <= ltp_diff_ce
        else:
            ltp_diff_ce = max_ltp_ce - min_ltp_ce
            ltp_ce_check = 0.4 < ltp_diff_ce
        ...
        if ltp_pe_check:
            ...
            #print(ltp_diff_pe)
            ltp_pe_check = 0.25 < ltp_diff_pe
        else:
            ltp_diff_pe = max_ltp_pe - min_ltp_pe
            ltp_pe_check = 0.4 < ltp_diff_pe

        l=[]
        l.append(ltp_diff < 5)
        l.append(ltp_diff_ce < 3)
        l.append(ltp_diff_pe < 3)
        ...
        if (abs(ltp_price) > 15 or abs(prev_ltp_price) > 20) and sum(l) >= 2:
            ...
            '''ltp_ni_check = (ltp_price > 0 and bid_pr > 0 and ask_pr > 0) or (ltp_price < 0 and bid_pr < 0 and ask_pr < 0)
            min_ltp = min(abs(ltp_price), abs(bid_pr), abs(ask_pr))
            max_ltp = max(abs(ltp_price), abs(bid_pr), abs(ask_pr))
            if ltp_ni_check:
                ltp_diff = max_ltp - min_ltp
            ...
            ltp_ce_check = (ltp_price_ce > 0 and bid_pr_ce > 0 and ask_pr_ce > 0) or (ltp_price_ce < 0 and bid_pr_ce < 0 and ask_pr_ce < 0)
            min_ltp_ce = min(abs(ltp_price_ce), abs(bid_pr_ce), abs(ask_pr_ce))
            max_ltp_ce = max(abs(ltp_price_ce), abs(bid_pr_ce), abs(ask_pr_ce))
            if ltp_ce_check:
                ltp_diff_ce = max_ltp_ce - min_ltp_ce
                ltp_ce_check = 0.25 < ltp_diff_ce < 3

            ltp_pe_check = (ltp_price_pe > 0 and bid_pr_pe > 0 and ask_pr_pe > 0) or (ltp_price_pe < 0 and bid_pr_pe < 0 and ask_pr_pe < 0)
            min_ltp_pe = min(abs(ltp_price_pe), abs(bid_pr_pe), abs(ask_pr_pe))
            max_ltp_pe = max(abs(ltp_price_pe), abs(bid_pr_pe), abs(ask_pr_pe))
            if ltp_pe_check:
                ltp_diff_pe = max_ltp_pe - min_ltp_pe

                ltp_pe_check = 0.25 < ltp_diff_pe < 3

        else:
            ltp_ni_check, ltp_ce_check, ltp_pe_check = False, False, False'''

        '''if abs(ltp_price) > 15 or abs(prev_ltp_price) > 20:

            ltp_ni_check_less = abs(ltp_price) < min(abs(bid_pr), abs(ask_pr))
            if ltp_ni_check_less:
                min_bid_ask = max(abs(bid_pr), abs(ask_pr))
                ltp_diff = abs(ltp_price) - min_bid_ask
                ltp_ni_check_less = 0.9 < ltp_diff < 5
            else:
                ltp_ni_check_less = False
            ltp_ce_check_less = abs(ltp_price_ce) < min(abs(bid_pr_ce), abs(ask_pr_ce))
            if ltp_ce_check_less:
                min_bid_ask_ce = max(abs(bid_pr_ce), abs(ask_pr_ce))
                ltp_diff_ce = abs(ltp_price_ce) - min_bid_ask_ce
                ltp_ce_check_less = 0.45 < ltp_diff_ce < 3
            else:
                ltp_ce_check_less = False
            ltp_pe_check_less = abs(ltp_price_pe) < min(abs(bid_pr_pe), abs(ask_pr_pe))
            if ltp_pe_check_less:
                min_bid_ask_pe = max(abs(bid_pr_pe), abs(ask_pr_pe))
                ltp_diff_pe = abs(ltp_price_pe) - min_bid_ask_pe
                ltp_pe_check_less = 0.45 < ltp_diff_pe < 3
            else:
                ltp_pe_check_less = False
        else:
            ltp_ni_check_less, ltp_ce_check_less, ltp_pe_check_less = False, False, False'''
        ...
        contract_signal_power_b = (contract_signal == 1 and signal_power > 1.5) or signal_power > 2
        contract_signal_power_s = (contract_signal == -1 and signal_power > 1.5) or signal_power > 2
        ...
        if (ltp_check or ltp_ni_second_check) and count_Y > 3 and contract_signal_power_b and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
        ...
        elif (ltp_check or ltp_ni_second_check) and count_Y == 3 and count_N >= 1 and contract_signal_power_b and (((buy_sum > prev_buy_sum) and (sell_sum < prev_sell_sum)) or (buy_sum > 0 and sell_sum < 0)) \
            and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
            (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))):
        ...
        if not buy_flag:
            if (ltp_check or ltp_ni_second_check) and count_YES > 3  and contract_signal_power_s and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
                print(f"{current_row['last_traded_time']} - sell")

            elif (ltp_check or ltp_ni_second_check) and count_YES == 3 and count_N >= 1  and contract_signal_power_s and (((buy_sum < prev_buy_sum) and (sell_sum > prev_sell_sum)) or (buy_sum < 0 and sell_sum > 0)) \
                and ((buy_sum * prev_buy_sum < 0) or (len(str(abs(prev_buy_sum))) < len(str(abs(buy_sum)))) or 
                    (sell_sum * prev_sell_sum < 0) or (len(str(abs(prev_sell_sum))) < len(str(abs(sell_sum))))) :
