...
df_features['last_traded_time'] = pd.to_datetime(df_features['last_traded_time'])
...
# Normalize features
scaler = MinMaxScaler()
feature_cols = [col for col in df_features.columns if col != target and col != 'last_traded_time']
...
for i in range(len(df) - window_size):
    window = df_features[feature_cols].iloc[i:i + window_size].values
...
timestamps = df_features['last_traded_time'].iloc[window_size:].values
timestamps_resampled = timestamps[ros.sample_indices_]

# === 5. Train/Validation/Test Split ===
X_train, X_temp, y_train, y_temp, timestamps_train, timestamps_temp = train_test_split(
    X_resampled, y_resampled, timestamps_resampled, test_size=0.3, random_state=42, stratify=y_resampled)
...
for ts, pred, actual in zip(timestamps_test, y_test_prediction, y_test):
    pred_class = np.argmax(pred)
    formatted_time = pd.to_datetime(ts).strftime('%Y-%m-%d %H:%M:%S')
    print(f"Time: {formatted_time}, Prediction: {pred}")
...
"""
new_df = pd.read_csv("new_aggregated_master.csv")  # replace with your actual file
new_df['last_traded_time'] = pd.to_datetime(new_df['last_traded_time'], dayfirst=True)

# Keep a copy of time column if you want to match predictions later
timestamps_new = new_df['last_traded_time'].values

# === Preprocessing: Encode and Normalize ===
categorical_cols = new_df.select_dtypes(include=['object', 'string']).columns
for col in categorical_cols:
    new_df[col] = LabelEncoder().fit_transform(new_df[col])  # USE SAVED ENCODERS FOR REAL INFERENCE!

scaler = MinMaxScaler()
feature_cols = [col for col in new_df.columns if col != 'last_traded_time']
new_df[feature_cols] = scaler.fit_transform(new_df[feature_cols])  # USE SAVED SCALER FOR REAL INFERENCE!

# === Create Sliding Windows ===
window_size = 75
X_new = []
for i in range(len(new_df) - window_size):
    window = new_df[feature_cols].iloc[i:i + window_size].values
    X_new.append(window)
...
# === Predict ===
predictions = model.predict_with_thresholds(X_new)

# === Optional: Decode Labels ===
# If needed, reconstruct the label encoder
original_classes = ['class1', 'class2', 'class3']  # <- replace with your actual class names
label_encoder = LabelEncoder()
label_encoder.fit(original_classes)
decoded_preds = label_encoder.inverse_transform(predictions)

# === Output Predictions ===
for i, (time, pred) in enumerate(zip(timestamps_new[window_size:], decoded_preds)):
    print(f"{pd.to_datetime(time)} -> Predicted Class: {pred}")"""