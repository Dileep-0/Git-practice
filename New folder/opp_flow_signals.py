...
if all(column in df.columns for column in required_columns):
    ...
    for i in range(1, len(df)):
        ...
        if (ltp_check or ltp_check_2) and count_Y > 3 and (for_buy or buy_2 or buy_3) and contract_signal_power_b and ((buy_sum < 0) and (sell_sum < 0)) and ((change_in_buy < 0) and (change_in_sell < 0))\
            and ((buy_sum < change_in_buy) and (sell_sum > change_in_sell)) :
            print(f"{current_row['last_traded_time']} - buy {pattern}")
            #print(ltp_price_ce/ltp_price_pe)

        elif (ltp_check or ltp_check_2) and count_Y == 3 and count_N >= 1 and (for_buy or buy_2 or buy_3) and contract_signal_power_b and ((buy_sum < 0) and (sell_sum < 0)) and ((change_in_buy < 0) and (change_in_sell < 0))\
            and ((buy_sum < change_in_buy) and (sell_sum > change_in_sell)) :
        ...
        if not buy_flag:
            if (ltp_check or ltp_check_2) and count_YES > 3  and (for_sell or sell_2 or sell_3 ) and contract_signal_power_s and ((buy_sum > 0) and (sell_sum > 0)) and ((change_in_buy > 0) and (change_in_sell > 0))\
            and ((buy_sum > change_in_buy) and (sell_sum < change_in_sell)) :
                print(f"{current_row['last_traded_time']} - sell {pattern}")
                #print(ltp_price_ce/ltp_price_pe)

            elif (ltp_check or ltp_check_2) and count_YES == 3 and count_N >= 1  and (for_sell or sell_2 or sell_3 ) and contract_signal_power_s and ((buy_sum > 0) and (sell_sum > 0)) and ((change_in_buy > 0) and (change_in_sell > 0))\
            and ((buy_sum > change_in_buy) and (sell_sum < change_in_sell)) :
